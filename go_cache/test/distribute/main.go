...
func createGroup() *go_cache.Group {
	return go_cache.NewGroup("scores",
		2<<10,
		go_cache.GetterFunc(func(key string) ([]byte, error) {
			slog.Debug("[SlowDB] search key" + key)
			if v, ok := db[key]; ok {
				return []byte(v), nil
			}
...
func startCacheServer(addr string, addrs []string, gee *go_cache.Group) {
	slog.Debug("begin start CacheServer: " + addr)
	peers := go_cache.NewHTTPPool(addr)
	peers.Set(addrs...)
	gee.RegisterPeers(peers)
	slog.Info("geecache is running at" + addr)
	log.Fatal(http.ListenAndServe(addr[7:], peers))
}

func startAPIServer(apiAddr string, gee *go_cache.Group) {
	slog.Debug("begin start APIServer")
...
func main() {
	...
	slog.Debug(fmt.Sprintln(port, api))
